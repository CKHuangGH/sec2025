...
for (( times=0; times<7; times++ )); do
    ...
    do 
	    ssh root@$ip bash /root/sec2025/federation_framework/scenario1/liqo/scenario1/script/deployprometheus-member.sh
    done
    sleep 60
    ...
    for ip in $(cat node_exec)
    do 
	    ssh root@$ip python3 /root/sec2025/federation_framework/scenario1/liqo/scenario1/script/getmetrics_cpuram_time_member.py
    done
    sleep 10
    ...
    for ip in $(cat node_exec)
    do 
	    ssh root@$ip bash /root/sec2025/federation_framework/scenario1/liqo/scenario1/script/reset_worker.sh $number
        ssh root@$ip bash /root/sec2025/federation_framework/scenario1/liqo/scenario1/script/deleteprometheus.sh 
    done
    rm -rf results
    sleep 60
done